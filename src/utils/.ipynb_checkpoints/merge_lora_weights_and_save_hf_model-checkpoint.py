...
def main():
    ...
    full_sd = torch.load(model_args.model_with_lora, map_location="cpu")
    # keep only the adapters (and any non‑zero biases or norms)
    lora_sd = {k: v for k, v in full_sd.items() if v.numel() > 0}
